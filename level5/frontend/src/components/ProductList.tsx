              ...
              <React.Fragment key={product.id}>
                <ListItem secondaryAction={
                    <>
                      ...
                      <Button onClick={() => handleFetchProductUsers(product.id)}>
                        {expandedProducts[product.id] ? 'Hide Users' : 'View Users'}
                      </Button>
                    </>
                  }>
                  <ListItemText primary={product.name} />
                </ListItem>
                <Collapse in={expandedProducts[product.id]} timeout="auto" unmountOnExit>
                  <List component="div" disablePadding style={{ paddingLeft: '20px' }}>
                    {productUsers[product.id] && productUsers[product.id].map(user => (
